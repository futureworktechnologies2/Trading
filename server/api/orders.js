                    ...
                    var balance = user.wallet - player_worth;
                        ...
                        Inventory.findOne({'user_id': ObjectId(user_id), 'player_id' : ObjectId(other_order.player_id) }, function(err, inventory) {
                            ...
                            if(other_order.quantity > player_quantity )
                            {
                                ...
                                Order.findOne({ 'side' : 'SELL', 'user_id': ObjectId(other_order.user_id), 'player_id': ObjectId(other_order.player_id)}, function(err, update_other_order) {
                                ...
                                {
                                    Order.findOne({ 'side' : 'BUY', 'user_id': ObjectId(user_id), 'player_id': ObjectId(order.player_id)}, function(err, update_order) {
                        	...
                        	quantity_remains = other_order.quantity - player_quantity
                            }
                            else
                            {
                                // delete the order
                                ...
                                // update the order quantity
                                Order.findById({ 'side' : 'BUY', 'user_id': ObjectId(user_id), 'player_id': ObjectId(order.player_id)}, function(err, update_order) {
                                    if(update_order != null)
                                    {
                                        // quantity_remains = update_order.quantity - player_quantity
                                        update_order.quantity = update_order.quantity - player_quantity;
                        	...
                        	quantity_remains = order.quantity - player_quantity
                            }
