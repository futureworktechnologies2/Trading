 ...
 * @param {type} param2
 */
app.controller('PostCtrl', function($scope, Posts, Pages,Portfolios,Categories, $location) {
    console.log("home js controller PostCtrl")
    $scope.url = {};
    ...
    Categories.sigledata($scope.url).then(function(res) {
        if (res == null) {
            ...
            console.log("in the post controller start..");
            console.log(res);
            console.log("in the post controller end..");
            $scope.description = res.description;
            $scope.himage = 'aaaaaaa';
            // $scope.himage = res.himage;
            // $scope.simage = res.simage;
            $scope.title = res.title;
        }
